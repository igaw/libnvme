...
#define add_hex_argument(o, argstr, tok, arg, allow_zero) 		\
({									\
	int ret;							\
	if (nvmf_check_option(r, tok)) {				\
...
static int build_options(nvme_host_t h, nvme_ctrl_t c, char **argstr)
{
	...
	if (key_id == 0) {
		if (cfg->tls_retained_key)
			key_id = cfg->tls_retained_key;
		else 
			key_id = cfg->tls_key;
	}
		  ...
		  const struct nvme_fabrics_config *cfg)
{
	...
	if (s) {
		...
		if (fc) {
			...
			key = nvme_ctrl_get_tls_retained_key(fc);
			if (key)
				nvme_ctrl_set_tls_retained_key(c, key);
