...
const char *nvme_ctrl_get_tls_retained_key(nvme_ctrl_t c)
{
	return c->tls_retained_key;
}

void nvme_ctrl_set_tls_retained_key(nvme_ctrl_t c, const char *key)
{
	if (c->tls_retained_key) {
		free(c->tls_retained_key);
		c->tls_retained_key = NULL;
	}
	if (key)
		c->tls_retained_key = strdup(key);
}
...
void nvme_deconfigure_ctrl(nvme_ctrl_t c)
{
	...
	FREE_CTRL_ATTR(c->tls_retained_key);
